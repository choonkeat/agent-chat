...
- **`ERR_CONNECTION_RESET` on every `page.goto` means the tunnel is refusing ephemeral ports.** The CDP browser reaches this container through a tunnel that reliably forwards only the preview range (`SWE_PREVIEW_PORTS`, typically 3000-3019). Specs start their server on port 0, so when the tunnel is in that state every test fails before it reaches the app — including tests unrelated to the change. Pin the port to run the suite: `E2E_SERVER_PORT=3003 npx playwright test e2e/paste-upload.spec.cjs` (any free port in the range). Only `paste-upload.spec.cjs` honours `E2E_SERVER_PORT` today.
